...
**/
enum FireMode1 { semi, auto }
var Mode = FireMode1.semi;
...
function OnGUI (){
	if(selected){
		...
		if(selected){
			...
			GUI.Label (Rect(Screen.width - 85,Screen.height-535,200,80),"" + bulletsLeft, style1);
			GUI.Label (Rect(Screen.width - 70,Screen.height-535,200,80)," / " + magazines, style2);
			
			//for health score
		//GUI.Label(new Rect(SH * 2.05f, SH * 0.30f,SH * 0.200f, SH * 0.200f), ""+ bulletsLeft , style1);
		//GUI.Label(new Rect(SH * 2.10f, SH * 0.30f,SH * 0.200f, SH * 0.200f), " / " + magazines, style2);
		...
		if(scopeTexture != null && inScope){
			...
			if(crosshairFirstModeHorizontal != null){	
				...
				var position1 = Rect((Screen.width + w)/2 + (triggerTime * adjustMaxCroshairSize),(Screen.height - h)/2, w, h);
				var position2 = Rect((Screen.width - w)/2,(Screen.height + h)/2 + (triggerTime * adjustMaxCroshairSize), w, h);
				var position3 = Rect((Screen.width - w)/2 - (triggerTime * adjustMaxCroshairSize) - w,(Screen.height - h )/2, w, h);
				var position4 = Rect((Screen.width - w)/2,(Screen.height - h)/2 - (triggerTime * adjustMaxCroshairSize) - h, w, h);
				if (!aiming) { 
