	...
	if(selected){
		...
		if(ammoMode == Ammo.Magazines){
			if(firstMode != fireMode.none && firstMode != fireMode.launcher || secondMode != fireMode.none && secondMode != fireMode.launcher){
				GUI.Label (Rect(Screen.width - 60,Screen.height-450,200,80),"Ammo : ");
				GUI.Label (Rect(Screen.width - 60,Screen.height-450,200,80),"" + bulletsLeft, style1);
				GUI.Label (Rect(Screen.width - 50,Screen.height-450,200,80)," / " + magazines);
			}
		}	
		...
		if(crosshairSecondMode != null){
			if(currentMode == fireMode.launcher){
				...
				var position = Rect((Screen.width - width)/2,(Screen.height - height )/2, width, height);
				if (!aiming) { 
